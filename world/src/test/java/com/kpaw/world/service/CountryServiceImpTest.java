...
class CountryServiceImpTest {
    ...
    @Test
    void searchBy() {
        given(countryRepository.findByCodeContainsAndNameContainsAndRegionContainsAllIgnoreCase("code", "name", "region")).willReturn(countries);
        List<Country> foundCountries = service.searchBy("code", "name", "region");
        then(countryRepository).should().findByCodeContainsAndNameContainsAndRegionContainsAllIgnoreCase("code", "name", "region");
        assertThat(foundCountries).isNotNull();
    }
